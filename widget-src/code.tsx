...
function Widget() {
  ...
  const widgetId = useWidgetId()
   ...
   /// COLORS ///
   const black100 = '#111111';
   const black80 = '#111111cc';
   const black10 = '#1111111A';
   const todiscussColor = '#FFFFFF';
   ...
   <path d="M9.25 10.75H5V9.25H9.25V5H10.75V9.25H15V10.75H10.75V15H9.25V10.75Z" fill="#111111"/>
 </svg>`;

  /// PROGRESSION ///
  const [progression, setProgression] = useSyncedState('progression', 'to discuss');
  const progressionOptions = [{ option: 'to discuss', label: 'To discuss'}, { option: 'to do', label: 'To do'}, { option: 'in progress', label: 'In progress'}, { option: 'done', label: 'Done'},{ option: 'cancelled', label: 'Cancelled'}];
  const [progressionColor, setProgressionColor] = useSyncedState('progressionColor', todiscussColor);
  
  let text_decoration;
  progression === 'cancelled' ? text_decoration = 'strikethrough': text_decoration = 'none';

  let cardOpacity;
  progression === 'cancelled' ? cardOpacity = 0.7 : cardOpacity = 1;
  ...
  const priorityOptions = [{ option: 'MUST', label: 'Must have'}, { option: 'SHOULD', label: 'Should have'}, { option: 'COULD', label: 'Could have'},{ option: `WON'T`, label: 'Wont have'}];

  const priorityIconsJSX = (priority) => {
    let opacity;
    if (priority === 'MUST') opacity = [1,1,1]
    if (priority === 'SHOULD') opacity = [1,1,0.08]
    if (priority === 'COULD') opacity = [1,0.08,0.08]
    if (priority === `WON'T`) opacity = [0.08,0.08,0.08]
    return (
            <AutoLayout
              verticalAlignItems={'center'}
            >
              <Text fontFamily="Inter" fontSize={24} opacity={opacity[0]} lineHeight={24}>♥</Text>
              <Text fontFamily="Inter" fontSize={24} opacity={opacity[1]} lineHeight={24}>♥</Text>
              <Text fontFamily="Inter" fontSize={24} opacity={opacity[2]} lineHeight={24}>♥</Text>
  ...
  const complexityOptions = [{ option: 'S', label: 'S - Easy'}, { option: 'M', label: 'M - Moderate'}, { option: 'L', label: 'L - Challenging'},{ option: 'XL', label: 'XL - Complex'}];
  //const [complexityColor, setComplexityColor] = useSyncedState('complexityColor', '#219653') 
